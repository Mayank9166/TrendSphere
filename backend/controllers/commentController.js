...
export const getComments = async(req,res,next)=>{
   ...
   // If user is admin, return all comments
   if(req.user.isAdmin) {
     console.log('✅ User is admin, returning all comments');
     try {
     ...
     // If user is not admin, return only their own comments
     console.log('✅ User is not admin, returning own comments');
     try {
       const startIndex = parseInt(req.query.startIndex)||0;
       const limit = parseInt(req.query.limit)||9;
       const sortDirection = req.query.sort === "desc"?-1:1;
       const comments = await Comment.find({ userId: req.user.id }).sort({createdAt:sortDirection}).skip(startIndex).limit(limit);
